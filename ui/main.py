...
def prepare_environment():
    roop.globals.output_path = os.path.abspath(
...
def run():
    ...
    print(
        f'Đang sử dụng {roop.globals.execution_providers} - Thiết bị:{util.get_device()}')
    ...
    while run_server:
        ...
        with gr.Blocks(title=f'{roop.metadata.name} {roop.metadata.version}', theme=roop.globals.CFG.selected_theme, css=mycss) as ui:
            with gr.Row(variant='compact'):
                gr.Markdown(
                    f"### [{roop.metadata.name} {roop.metadata.version}](https://github.com/tripleseven190504/roop-unleashed)")
        ...
        uii.ui_restart_server = False
        try:
            ui.queue().launch(inbrowser=True, server_name=server_name, server_port=server_port,
                              share=roop.globals.CFG.server_share, ssl_verify=ssl_verify, prevent_thread_lock=True, show_error=True)
        except Exception as e:
            print(f'Exception {e} khi khởi động Máy chủ Gradio!')
